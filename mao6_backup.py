...
def checkjson():
	...
	try:
		...
		return direction, intensity, intensity2
	except:
		return "down", 0, 0
...
def updatePlayer():
	(direction, intensity, intensity2) = checkjson()
	if (direction=="up"):
		art.setZ(avatar.getZ()+abs(intensity))
		#avatar.setZ(art.getY()+abs(intensity))
		art2.setZ(avatar2.getZ()+abs(intensity2))
		#avatar2.setY(art2.getY()+abs(intensity2))
...
# since our avatar origin is centered in a model sized 2,2,2, we need to shift it 1 unit above the ground and this time we make this happen shifting it off its own root node (avatarNP)
avatar2.setPos(art.getX(),art.getY(), art.getZ()+3.6)
...
# of course we mark it with the wall mask
artCollider.node().setFromCollideMask(WALL_MASK)


# we reintroduced in this snippet the renowned smiley collision sphere - we need it as low-poly collision geometry for the wall collision handler to know when the smiley hit a wall.
avatar2Collider = avatar2.attachNewNode(CollisionNode('smileycnode'))
avatar2Collider.node().addSolid(CollisionSphere(art.getX(),art.getY(), art.getZ()+3.6, 1))
# of course we mark it with the wall mask
avatar2Collider.node().setFromCollideMask(WALL_MASK)
...
# of course we mark it with the wall mask
art2Collider.node().setFromCollideMask(WALL_MASK)
